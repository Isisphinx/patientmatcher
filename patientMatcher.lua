...
function OnStableStudy(studyOrthancID, tags, metadata)
    ...
    local modifiedStudyOrthancID = ParseJson(RestApiPost('/studies/' .. studyOrthancID .. '/modify',
        DumpJson(studyCommand, true)))['ID']
    ...
    local modifiedPatientOrthancID = ParseJson(RestApiPost('/patients/' .. patientOrthancID .. '/modify',
        DumpJson(patientCommand, true)))['ID']

    -- Modify the studyuid and send to peers

    local newStudyOrthancId = ReturnMatchingStudy(modifiedPatientOrthancID, matcherResponse['studyid'])
    
    local studyUIDReplace = {}
    studyUIDReplace['StudyInstanceUID'] = matcherResponse['studyinstanceuid']
    local studyUIDCommand = {}
    studyUIDCommand['Replace'] = studyUIDReplace
    studyUIDCommand['Force'] = true
    studyUIDCommand['KeepSource'] = false
    local modifiedStudyUIDOrthancID = ParseJson(RestApiPost('/studies/' .. newStudyOrthancId .. '/modify',
        DumpJson(studyUIDCommand, true)))['ID']

    -- Send rectified study to peers
    SendToPeers(modifiedStudyUIDOrthancID)
end
