...
function RequestMatcher(Ip, Port, rawPatientName, rawPatientBirthDate, rawStudyDate)
    ...
    SetHttpTimeout(1)
    local matcherResponse = HttpGet("http://" ..
    Ip .. ':' .. Port .. "/study/" .. PatientBirthDate .. "/" .. PatientName .. "/" .. StudyDate)
...
function ReturnMatchingStudy(patientOrthancID, targetStudyID)
    -- Get patient details
    local patientDetails = ParseJson(RestApiGet('/patients/' .. patientOrthancID))
    local matchedStudyID
